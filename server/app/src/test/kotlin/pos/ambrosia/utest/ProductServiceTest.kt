...
class ProductServiceTest {
    ...
    fun `addProduct returns null if SKU already exists`() {
        runBlocking {
            val newProduct = Product(null, "SKU-1", "Prod1", null, null, 100, "cat-1", 5, 199) // Arrange
            whenever(mockConnection.prepareStatement(any())).thenReturn(mockStatement) // Arrange
            whenever(mockStatement.executeQuery()).thenReturn(mockResultSet) // Arrange
            whenever(mockResultSet.next()).thenReturn(true) // Arrange
    ...
    fun `addProduct returns new ID on success`() {
        runBlocking {
            val newProduct = Product(null, "SKU-NEW", "New Product", null, null, 100, "cat-1", 5, 199) // Arrange
    ...
    fun `updateProduct returns false if ID is null`() {
        runBlocking {
            val productWithNullId = Product(id = null, SKU = "SKU-1", name = "Name", description = null, image_url = null, cost_cents = 100, category_id = "cat-1", quantity = 1, price_cents = 100) // Arrange
            val service = ProductService(mockConnection) // Arrange
    ...
    fun `updateProduct returns false if invalid data`() {
        runBlocking {
            val invalid = Product(id = "p-1", SKU = "", name = " ", description = null, image_url = null, cost_cents = -1, category_id = "", quantity = -1, price_cents = -1) // Arrange
            val service = ProductService(mockConnection) // Arrange
    ...
    fun `updateProduct returns false if SKU belongs to another product`() {
        runBlocking {
            val toUpdate = Product(id = "p-1", SKU = "SKU-TAKEN", name = "New Name", description = null, image_url = null, cost_cents = 100, category_id = "cat-1", quantity = 5, price_cents = 250) // Arrange
            val skuCheckStatement: PreparedStatement = mock() // Arrange
            whenever(mockConnection.prepareStatement(contains("WHERE SKU = ?"))).thenReturn(skuCheckStatement) // Arrange
            val skuCheckResultSet: ResultSet = mock() // Arrange
    ...
    fun `updateProduct returns true on success`() {
        runBlocking {
            val toUpdate = Product(id = "p-1", SKU = "SKU-OK", name = "Updated", description = null, image_url = null, cost_cents = 100, category_id = "cat-1", quantity = 5, price_cents = 250) // Arrange
            val skuCheckStatement: PreparedStatement = mock() // Arrange
