...
class ProductCategoryServiceTest {
    ...
    fun `updateCategory returns true on success`() {
        runBlocking {
            ...
            whenever(mockConnection.prepareStatement(contains("SELECT id FROM categories"))).thenReturn(checkNameStatement)
            whenever(mockConnection.prepareStatement(contains("UPDATE categories SET name"))).thenReturn(updateStatement)
    ...
    fun `deleteCategory returns false if in use`() {
        runBlocking {
            ...
            assertFalse(ok)
            verify(mockConnection, never()).prepareStatement(contains("UPDATE categories SET is_deleted"))
        }
    }
