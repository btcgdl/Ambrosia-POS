...
class ProductCategoryService(private val connection: Connection) {
  companion object {
    private const val ADD_CATEGORY = "INSERT INTO product_categories (id, name) VALUES (?, ?)"
    private const val GET_CATEGORIES = "SELECT id, name FROM product_categories WHERE is_deleted = 0"
    private const val GET_CATEGORY_BY_ID = "SELECT id, name FROM product_categories WHERE id = ? AND is_deleted = 0"
    private const val UPDATE_CATEGORY = "UPDATE product_categories SET name = ? WHERE id = ?"
    private const val DELETE_CATEGORY = "UPDATE product_categories SET is_deleted = 1 WHERE id = ?"
    private const val CHECK_CATEGORY_IN_USE = "SELECT COUNT(*) as count FROM products WHERE category_id = ? AND is_deleted = 0"
    private const val CHECK_NAME_EXISTS = "SELECT id FROM product_categories WHERE name = ? AND is_deleted = 0 AND id != ?"
  }
