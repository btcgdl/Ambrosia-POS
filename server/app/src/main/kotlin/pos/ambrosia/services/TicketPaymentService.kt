...
class TicketPaymentService(private val connection: Connection) {
    companion object {
    ...
    suspend fun getTicketPaymentsByTicket(
            ticketId: String
    ): List<pos.ambrosia.models.TicketPayment> {
    ...
    suspend fun getTicketPaymentsByPayment(
            paymentId: String
    ): List<pos.ambrosia.models.TicketPayment> {
    ...
    suspend fun deleteTicketPayment(paymentId: String, ticketId: String): Boolean {
        ...
        if (rowsDeleted > 0) {
            logger.info(
                    "Ticket payment deleted successfully: payment $paymentId -> ticket $ticketId"
            )
        } else {
