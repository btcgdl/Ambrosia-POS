...
class CategoryService(private val connection: Connection) {
  ...
  private fun usageTable(type: String): String = when (type) {
    "dish" -> "dishes"
  ...
  private fun map(rs: java.sql.ResultSet): CategoryItem {
    return CategoryItem(id = rs.getString("id"), name = rs.getString("name"))
  }

  private fun nameExists(name: String, type: String, excludeId: String? = null): Boolean {
  ...
  private fun categoryInUse(categoryId: String, type: String): Boolean {
  ...
  suspend fun addCategory(type: String, category: CategoryItem): String? {
  ...
  suspend fun getCategoryById(id: String, type: String): CategoryItem? {
    if (!validateType(type)) return null
    val st = connection.prepareStatement(
  ...
  suspend fun updateCategory(type: String, category: CategoryItem): Boolean {
  ...
  suspend fun deleteCategory(id: String, type: String): Boolean {
    if (!validateType(type)) return false
