...
export default function Restocks() {
  ...
  const handleSubmitRestock = (e) => {
    ...
    if (formRestock.supplierId && formRestock.items.length > 0) {
      ...
      if (new Set(ingredientIds).size !== ingredientIds.length) {
        alert(
          "No se pueden incluir ingredientes duplicados en el reabastecimiento.",
        );
        return;
      }
      // Calcular totalCost
      ...
      const validItems = formRestock.items
        .filter((item) => item.ingredientId && item.quantity && item.cost)
        .map((item) => ({
  ...
  const supplier = suppliers.find(
    (sup) => sup.id === Number(formRestock.supplierId),
  );
      ...
      <div className="h-[90%] w-[90%] bg-amber-100 rounded-xl p-6 flex flex-col items-center gap-6 overflow-y-auto">
                ...
                {formRestock.items.map((item, index) => (
                  ...
                  >
                    <select
                      ...
                      onChange={(e) =>
                        handleChangeItem(index, "ingredientId", e.target.value)
                      }
                    ...
                    <input
                      ...
                      value={item.cost}
                      onChange={(e) =>
                        handleChangeItem(index, "cost", e.target.value)
                      }
                      placeholder="Costo"
                      className="text-2xl p-3 rounded-lg w-1/4"
                      required
        ...
        <div className="w-full max-w-4xl space-y-4">
          {restocks.map((restock) => (
            ...
            >
              <div>
                <strong>Reabastecimiento #{restock.id}</strong> – Proveedor:{" "}
                {suppliers.find((sup) => sup.id === restock.supplierId)?.name},
                Ingredientes: {restock.items.length}, Costo: $
                {restock.totalCost}, Fecha: {restock.date}
              </div>
              <button
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-amber-100 p-6 rounded-xl max-w-2xl w-full max-h-[80vh] overflow-y-auto">
              <h3 className="text-3xl font-bold mb-4">
                Detalles del Reabastecimiento #{selectedRestock.id}
              </h3>
              <p className="text-2xl mb-2">
                <strong>Proveedor:</strong>{" "}
                {
                  suppliers.find((sup) => sup.id === selectedRestock.supplierId)
                    ?.name
                }
              </p>
              <p className="text-2xl mb-2">
