...
export default function Inventory() {
  ...
  const handleSubmitIngredient = (e) => {
    ...
    if (editingIngredient) {
      updateIngredient({
        ...formIngredient,
        id: editingIngredient,
        quantity: Number(formIngredient.quantity),
        lowStockThreshold: Number(formIngredient.lowStockThreshold),
        costPerUnit: Number(formIngredient.costPerUnit),
      });
    } else {
      addIngredient({
        ...formIngredient,
            ...
            {filteredIngredients.map((ingredient) => (
              ...
              >
                <div>
                  ...
                  {ingredient.quantity} {ingredient.unit}, Costo: $
                  {ingredient.costPerUnit}/unidad
            ...
            {restocks.map((restock) => (
              ...
              >
                <div>
                  <strong>
                    {
                      ingredients.find((ing) => ing.id === restock.ingredientId)
                        ?.name
                    }
                  </strong>{" "}
                  – {restock.quantity} unidades, Proveedor:{" "}
                  {suppliers.find((sup) => sup.id === restock.supplierId)?.name}
                  , Costo: ${restock.totalCost}, Fecha: {restock.date}
                </div>
              </div>
            ))}
          </div>
